...
function doPost(e) {
  try {
    ...
    } else if (action === 'responsableExportAttendanceBySubject') { // NOUVEAU
      return responsableExportAttendanceBySubject(data);
...
function addCourseForAdmin(data) {
    try {
        ...
        const { classe, matiere, enseignant, date, startTime, endTime } = payload;
        const newId = `CRS-${Utilities.getUuid().substring(0, 4).toUpperCase()}`;
        const newCourseRow = [newId, classe, matiere, enseignant, new Date(date), startTime, endTime, 'Confirmé'];
        ...
        logAction('adminAddCourse', { classe, matiere, universityId });

        return createJsonResponse({ success: true, message: `Le cours de ${matiere} pour la classe ${classe} a été ajouté au planning.` });
    } catch (error) {
...
function addCourseForResponsable(data) {
    try {
        ...
        const { matiere, enseignant, date, startTime, endTime } = payload;
        ...
        const newId = `CRS-${Utilities.getUuid().substring(0, 4).toUpperCase()}`;
        const newCourseRow = [newId, className, matiere, enseignant, new Date(date), startTime, endTime, 'En attente'];
        SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_NAMES.PLANNING).appendRow(newCourseRow);
        
        // Invalider les caches pertinents après l'ajout
        cache.remove(`dashboard_resp_${responsableId}`); // Cache du responsable
        clearAllCachesForUniversity(responsableRow[respHeaders.indexOf('ID_UNIVERSITE_FK')], ['planning']); // Cache de l'admin
        cache.remove(`planning_${responsableRow[respHeaders.indexOf('ID_UNIVERSITE_FK')]}`);
        logAction('addCourseForResponsable', { responsableId, matiere });

        return createJsonResponse({ success: true, message: `Le cours de ${matiere} a été ajouté au planning avec le statut "En attente".` });
    } catch (error) {
...
function recordAttendance(data) {
    try {
        ...
        SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_NAMES.SCAN).appendRow([timestamp, studentId, studentName, classe, matiere, Utilities.formatDate(timestamp, Session.getScriptTimeZone(), 'yyyy-MM-dd'), Utilities.formatDate(timestamp, Session.getScriptTimeZone(), 'HH:mm:ss'), 'Présent']);

        // Invalider le cache de l'historique de l'étudiant
        cache.remove(`attendance_history_${studentId.trim().toUpperCase()}`);
        logAction('recordAttendance', { studentId, classe, matiere });
...
function getCurrentCourse(data) {
  try {
    ...
    if ([classeIdx, matiereIdx, dateIdx, startIdx, endIdx, statutIdx].includes(-1)) {
...
function setup() {
  ...
  try {
    ...
    const sheetConfigs = {
      ...
      [SHEET_NAMES.PLANNING]: { headers: ['ID_COURS', 'CLASSE', 'MATIERE', 'ENSEIGNANT', 'DATE_COURS', 'HEURE_DEBUT', 'HEURE_FIN', 'STATUT'], color: '#0f9d58', validations: { 'STATUT': ['Confirmé', 'Annulé', 'En attente'] } },
      [SHEET_NAMES.SCAN]: { headers: ['TIMESTAMP', 'ID_ETUDIANT', 'NOM_ETUDIANT', 'CLASSE', 'MATIERE', 'DATE_SCAN', 'HEURE_SCAN', 'STATUT_PRESENCE'], color: '#db4437', validations: { 'STATUT_PRESENCE': ['Présent', 'Absent', 'En retard', 'Justifié'] } },
    ...
    const demoPlanning = [
      ['C001', 'L1-Info', 'Algorithmique', 'Prof. Ba', today, '08:00', '10:00', 'Confirmé'],
      ['C002', 'L1-Droit', 'Introduction au Droit', 'Prof. Ndiaye', today, '10:00', '12:00', 'Confirmé'],
      ['C003', 'L1-Info', 'Bases de données', 'Prof. Sow', tomorrow, '08:00', '10:00', 'En attente']
    ];
...
function updateSystem() {
  ...
  try {
    const sheetConfigs = {
      ...
      [SHEET_NAMES.PLANNING]: { headers: ['ID_COURS', 'CLASSE', 'MATIERE', 'ENSEIGNANT', 'DATE_COURS', 'HEURE_DEBUT', 'HEURE_FIN', 'STATUT'], color: '#0f9d58' },
      [SHEET_NAMES.SCAN]: { headers: ['TIMESTAMP', 'ID_ETUDIANT', 'NOM_ETUDIANT', 'CLASSE', 'MATIERE', 'DATE_SCAN', 'HEURE_SCAN', 'STATUT_PRESENCE'], color: '#db4437' },
      [SHEET_NAMES.CONDUCT]: { headers: ['ID_INCIDENT', 'DATE', 'ID_ETUDIANT', 'NOM_ETUDIANT', 'CLASSE', 'DESCRIPTION_INCIDENT', 'MESURE_PRISE'], color: '#673ab7' },
      [SHEET_NAMES.ACTION_LOG]: { headers: ['TIMESTAMP', 'ACTION', 'DONNEES', 'UTILISATEUR_IP'], color: '#78909c' },
      [SHEET_NAMES.ERROR_LOG]: { headers: ['TIMESTAMP', 'ACTION', 'REQUETE', 'MESSAGE_ERREUR', 'SUGGESTION_DEBUG', 'PILE_APPEL'], color: '#d50000' }
    };
...
function scanStudentForAttendance(data, ctx) {
  try {
    ...
    const matiere = courseResult.data.matiere;

    // 3. Enregistrer la présence (similaire à recordAttendance)
    return recordAttendance({ studentId, classe: className, matiere });
...
/**
 * NOUVEAU: Exporte la liste de présence pour une matière donnée pour un responsable.
 * @param {object} data - Contient { responsableId, matiere }.
 */
function responsableExportAttendanceBySubject(data) {
    try {
        const { responsableId, matiere } = data;
        if (!responsableId || !matiere) {
            throw new Error("ID du responsable et matière sont requis.");
        }
        ...
        const scanStudentIdIdx = scanHeaders.indexOf('ID_ETUDIANT');
        const scanClassIdx = scanHeaders.indexOf('CLASSE');
        const scanMatiereIdx = scanHeaders.indexOf('MATIERE');
        const attendanceCounts = scanData.slice(1)
            .filter(row => row[scanClassIdx] === className && row[scanMatiereIdx] === matiere)
            .reduce((acc, row) => {
                const studentId = row[scanStudentIdIdx];
        ...
        const fileName = `Presence_${className.replace(/\s/g, '_')}_${matiere.replace(/\s/g, '_')}.csv`;

        return createJsonResponse({ success: true, data: { csvContent, fileName } });
    } catch (error) {
        logError('responsableExportAttendanceBySubject', error);
        return createJsonResponse({ success: false, error: error.message });
    }
}
/**
